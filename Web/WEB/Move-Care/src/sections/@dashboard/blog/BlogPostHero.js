...
export default function BlogPostHero({ post }) {
  const { cover, title, author, createdAt,khach_hang,ngay_khoi_tao } = post;
  console.log(post.khach_hang_id.anh_dai_dien)
    ...
    >
      ...
      <StyledFooter>
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          <Avatar alt={khach_hang} src={post.khach_hang_id.anh_dai_dien} sx={{ width: 48, height: 48 }} />

          <Box sx={{ ml: 2 }}>
            <Typography variant="subtitle1" sx={{ color: 'common.white' }}>
              {post.khach_hang_id.ho_ten}
            </Typography>

            <Typography variant="body2" sx={{ color: 'grey.500' }}>
              {fDate(post.thoi_gian)}
            </Typography>
          </Box>
        </Box>
